...
diferencia quan ens troben en mode inserció o sobreescriptura.

Guarda el color actual del cursor en mode /insert/ per poder
restaurar-lo al alternar entre /overwrite/ i /insert/.

#+begin_src emacs-lisp :tangle yes
  (when (null my/theme-face-cursor-insert-background-color)
...
#+begin_src emacs-lisp :tangle yes
  (defun my/theme-update-cursor-color-for-insert-overwrite-mode ()
...
el problema de que al cambiar de buffer no s'ajustava el color del
...
#+begin_src emacs-lisp :tangle yes
  (add-hook 'post-command-hook #'my/theme-update-cursor-color-for-insert-overwrite-mode)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  En qualsevol cas, si l'arxiu és nou (buid) s'insereix el valor de
  la variable `my/current-project-TODO-template'."
    (interactive)
    (let ((current-project (project-current)))
      (if (or (null current-project)
          ...
          (let ((directori (or (locate-dominating-file default-directory my/current-project-TODO-path-fallback)
            ...
            (find-file (f-join directori my/current-project-TODO-path-fallback)))
        (find-file (f-join (project-root current-project) my/current-project-TODO-path)))
