...
La instal·lació està bastant automatitzada.

Al executar /emacs/ es generaran els fitxers =settings.el= i
=settings.sh=. El primer conté la configuració d'/emacs/ i el segon
comandes /sh/ per configurar l'entorn de treball.

* Introducció

La configuració d'emacs es guarda en un subdirectori de
=user-emacs-directory=. La constant =arv/emacs-conf-dir= permet
obtindre la ruta d'aquest directori.

Per utilitzar un directori de configuració diferent cal definir la
variable d'entorn =EMACS_CONFIG_DIR=.

Aquesta configuració requereix emacs 24.4 o posterior.

Explicar ací l'ùs d'una versió actualitzada de /org/ i que la
configuració hauria d'utilitzar elements compatibles amb /org/ 8.2.10
(inclosa en emacs 25.3)

El directori =site-lisp= s'utilitza per guardar codi extern no
disponible en ELPA. S'ha de minimitzar l'ús en la mesura del possible.
Si s'afegeixen nous mòduls cal actualitzar els /autoloads/:

#+begin_src emacs-lisp :tangle no
  (let* ((site-lisp-path (arv/path-concat arv/emacs-conf-dir "site-lisp"))
         (generated-autoload-file (arv/path-concat site-lisp-path "loaddefs.el")))
    (update-directory-autoloads site-lisp-path))
#+end_src

La configuració d'alguns paquets requereix instal·lar comandes
externes. Açò es documenta en cada paquet però al estar /desperdigat/
pot ser difícil de trobar. Mirar com es pot fer que aquesta informació
es pugui localitzar ràpidament. Es viable incloure les comandes
d'instal·lació en blocs de codi i /executar/ l'arxiu org?

En relació a això, definir unes recomanacions pel que fa als blocs de
codi de forma que la instal·lació sigui automatitzable.
...
Els bloc /sh/ s'utilitzen en dos contexts:

- s'insereixen en un arxiu per formar un únic script. Aquest script
  serà executat automàticament durant el primer arranc d'emacs.
...
Cal tindre molta cura al escriure aquests blocs de codi per tal de
garantir que funcionin en els dos contexts. Cada bloc de codi pot
assumir que el directori de treball és =~/.emacs.d/conf.d= i en
finalitzar ha de restaurar el directori de treball (utilitzant =pushd=
i =popd= enlloc de =cd=, per exemple).
...
La funció =arv/bind-keys= intenta fer menys verbosa la definició de
...
#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load "ebuff-menu"
...
En algun moment aço es mourà als =(use-package)= dels modes.

La variable =arv/diminished-modes= enumera els modes que es
/disminuiran/. Cada element de la llista té el format:
...
- =MODE=, simbol obligatori: és el mode
- =PACKAGE=, cadena opcional: nom del paquet, per defecte =MODE=
  eliminant el sufix =-mode=.
- =REPLACEMENT=, cadena opcional: text a mostra en la /modeline/, per
  defecte la cadena buida.

#+begin_src emacs-lisp :tangle yes
  (setq arv/diminished-modes
        '((abbrev-mode)
          (company-mode)
          (eldoc-mode)
...
Instrumentem /disminuir/ els modes quan es carrega el paquet
corresponent:
...
#+begin_src emacs-lisp :tangle yes
  (use-package diminish
    ...
    :commands diminish
    :config
    (dolist (item arv/diminished-modes)
      (let ((mode (car item))
            (package (or (cadr item)
                         (s-chop-suffix "-mode" (symbol-name (car item)))))
            (replacement (or (cl-third item) "")))
        ;; cal retrasar diminish fins que el corresponent paquet s'hagi
        ;; carregat i el mode es defineix.
...
#+begin_src emacs-lisp :tangle yes
  ...
  (defface arv/hi-red
    '((((min-colors 88)) (:weight bold :background "red1"))
...
La definició de colors probablement pertany a la configuració del
tema.
...
#+begin_src emacs-lisp :tangle yes
  (use-package ido
    ...
    :config
    (custom-set-faces
     '(ido-first-match ((t (:background "pale green" :foreground "black")))))
...
#+begin_src sh :tangle yes
  ...
  if [ ! -d org-mode ] ; then
      ...
      git --quiet clone https://code.orgmode.org/bzg/org-mode.git
      cd org-mode
      make local.mk
...
- gestió del projecte (en el sentit de django) i execució de comandes
  de gestió (=python-django=).

- operacions amb les /apps/: visitar un model, una vista, consultar la
  documentació etc. (=arv-py-django=).

- edició de plantilles (=pony-tpl=).

**** =arv-py-django=                                                 :manual:

https://github.com/patxoca/arv-py-django.git

Biblioteca que implementa funcions que faciliten la navegació dins un
projecte django. Depèn de =python-django=.

#+begin_src sh :tangle yes
  ./scripts/clone_git https://github.com/patxoca/arv-py-django.git
  ./scripts/compile_package arv-py-django
#+end_src

#+begin_src emacs-lisp :tangle yes
  (use-package arv-django
    :ensure nil
    :load-path "site-lisp/arv-py-django"
    :commands (arv/django-mode
               arv/django-switch-to-project-buffer))

  ;; TODO: aquest autoload apareixia en "setup-python-django.el" però no
  ;; tinc clar que sigui necessari. Es manté fins que puga fer alguna
  ;; prova. En funció del resultat caldrà afegir-lo a la clausula
  ;; ":commands" anterior o eliminar-lo.

  (autoload 'arv/django-file-belongs-to-app-p "arv-django" "" t nil)
#+end_src
...
**** =python-django=                                                 :manual:

https://github.com/fgallina/python-django.el

El codi sembla abandonat. S'ha incorporat una versió local dins el
directori =site-lisp= de la configuració. Resol problemes de
compatibilitat amb versions /recents/ (en relació al paquet) de
django.
...
#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load "python-django"
    (arv/bind-keys python-django-mode-map
                   '(("<left>" python-django-ui-move-up-tree))))

  (with-eval-after-load "python"
    (arv/bind-keys python-mode-map
                   '(("C-c j t" arv/django-jump-to-template)
                     ("C-c j j" arv/django-jump-to-javascript-controller))))

  (with-eval-after-load "sgml-mode"
    (arv/bind-keys html-mode-map
                   '(("C-c j t" arv/django-jump-to-template)
                     ("%" arv/django-autopair-template-tag)))
    (add-hook 'html-mode-hook
              (lambda ()
...
Variables i valors que es consideren segurs quan s'utilitzen en
variables locals al buffer:

#+begin_src emacs-lisp :tangle yes
  (customize-set-value 'safe-local-variable-values
                       '((encoding . utf-8)))
#+end_src
...
Canvia el color del cursor per indicar que ens troben en mode inserció
o sobreescriptura.
...
La implementació per defecte de la funció =grin= realitza la cerca en
el directori actual, el que no resulta massa pràctic. La funció
definida ací demana el directori on fer la cerca.
...
Al partir una finestra automàticament es selecciona un buffer diferent
(el següent) en la nova finestra. Amb un prefix conserva el
comportament original.

Vull veure com afecta al dia a dia. Variacions:

- utilitzar una altra funció per seleccionar el buffer (un buffer no
  visualitzat per exemple).

- permetre triar el buffer que es visualitza en la nova finestra,
  d'entre els existents.

- permetre visitar un fitxer.

- permetre triar si el canvi/visita es fa en la nova finestra
  (comportament actual) o en l'actual. Útil?

#+begin_src emacs-lisp :tangle yes
  (defun x-arv/vsplit-next-buffer (prefix)
    "Split the window vertically and display the next buffer."
    (interactive "P")
    (let ((new-window (split-window-below)))
      (unless prefix
        (switch-to-next-buffer new-window))))

  (defun x-arv/hsplit-next-buffer (prefix)
    "Split the window horizontally and display the next buffer."
    (interactive "P")
    (let ((new-window (split-window-right)))
      (unless prefix
        (switch-to-next-buffer new-window))))

  (global-set-key (kbd "C-x 2") #'x-arv/vsplit-next-buffer)
  (global-set-key (kbd "C-x 3") #'x-arv/hsplit-next-buffer)
#+end_src

** Provar =ibuffer=

No estic descontent amb =electric-buffer-list= però vaig a donar-li
una oportunitat a =ibuffer=.
...
- =define-compilation-mode=: en alguns mòduls utilitzo buffers de
  compilació per mostrar el resultat d'executar una comanda externa
  (p.e. llistar events i subscriptors en Hera). Mirar si aquesta macro
  aporta cap avantatge.

- ressaltar el mode insert/overwrite en la modeline utiltizant un
  color de fons. Actualment /insert/ no es visualitza.

- revisar les [[info:elisp#Coding%20Conventions][info:elisp#Coding Conventions]].

- [[https://melpa.org/#/redshank][redshank]]: Common Lisp Editing Extensions. Específicament m'interesa
  la part que permet refactoritzar codi.
