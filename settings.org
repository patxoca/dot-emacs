...
=elpy= configura la tecla =C-d= per mostrar la documentació quan
s'activa =company=, em resulta molt molest. Ho reverteixo:

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load "elpy"
    (advice-add 'elpy-module-company
                :around
                (lambda (oldfunc command &rest _args)
                  (let ((res (apply oldfunc command _args)))
                    (when (string-equal command "global-init")
