...
Originalment =my/bind-keys= es va definir per intentar fer menys
verbosa la definició de les tecles, però amb el temps ha esdevingut la
manera canònica de definir-les dins la configuració. Ha guanyat
funcionalitat: suporta la repetició de les comandes especificant el
/keyword/ =:repeat=.
...
#+begin_src emacs-lisp :tangle no
  (my/bind-keys some-mode-map
                '(("C-c f" do-foo)
                  ("C-c b" do-bar :repeat)
                  ("C-c s" do-spam :repeat "z")))
#+end_src

En aquest exemple:

- =C-c f= executarà la comanda =do-foo=

- =C-c b= executarà =do-bar=, que es pot repetir prement la tecla =b=.

- En el tercer cas s'especifica explícitament la tecla utilizada per
  repetir.

La funció =my/definie-repeatable-command= pot ser útil per crear
comandes que es poden repetir, i assignar-les a una tecla amb
=define-key= i companyia.

#+begin_src emacs-lisp :tangle yes
  (defun my/define-repeatable-command (key command)
    (let ((keymap (make-sparse-keymap))
          (fname (intern (concat (symbol-name command) "<repeat>"))))
      (fset fname
            `(lambda ()
  		     (interactive)
               (call-interactively (quote ,command))
  		     (set-transient-map (quote ,keymap) t)))
  	(define-key keymap key fname)
      fname))
#+end_src

Finalment =my/bind-keys=.
...
#+begin_src emacs-lisp :tangle yes
  (defun my/bind-keys (map key-definitions)
    (dolist (item key-definitions)
      (pcase item
        (`(,key ,command :repeat)
         (let ((vkey (kbd key)))  ;; vector de caràcters
           (define-key map vkey (my/define-repeatable-command (seq-subseq vkey -1) command))))
        (`(,key ,command :repeat ,repkey)
         (define-key map (kbd key) (my/define-repeatable-command (kbd repkey) command)))
        (`(,key ,command)
         (define-key map (kbd key) command))
  	  (_
         (warn "my/bind-key: format no suportat %s" item)))))
#+end_src

**** =my/host-reachable=                                         :deprecated:
:LOGBOOK:
- Note taken on [2025-08-20 dc. 07:38] \\
  No sembla que aquesta funció s'utilitzi enlloc. S'exclou del /tangle/.
:END:
...
**** =arv/generate-lab-faces=                                    :deprecated:
:LOGBOOK:
- Note taken on [2025-08-20 dc. 07:40] \\
  No sembla que aquesta funció s'utilitzi enlloc. S'exclou del /tangle/.
:END:
...
*** =beacon=                                                     :desactivat:
:LOGBOOK:
- Note taken on [2025-08-16 ds. 12:04] \\
  En algun moment, després d'alguna actualització d'Emacs, =beacon= va
  començar a ser molest; sembla que provoca efectes estranys quan
  s'activa al mateix temps que =company=. A més, no m'aporta gaire
  cosa, així que he optat per desactivar-lo.
:END:
...
El paquet /casual/ defineix menus /transient/ que faciliten
utilitzar/recordar les comandes de modes que utilitzo ocasionalment.

Per facilitar-ne l'us intentaré assignar a la tecla =F10= la comanda
=casual-xxx-tmenu= rellevant en cada cas.
...
https://github.com/kickingvegas/casual/blob/main/docs/bookmarks.org

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'bookmark
    (my/bind-keys bookmark-bmenu-mode-map
...
https://github.com/kickingvegas/casual/blob/main/docs/calc.org

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'calc
    (my/bind-keys calc-mode-map
                  '(("<f10>" casual-calc-tmenu)))
    )
  (with-eval-after-load 'calc-ext
    (my/bind-keys calc-alg-map
                  '(("<f10>" casual-calc-tmenu)))
    )
#+end_src
...
https://github.com/kickingvegas/casual/blob/main/docs/dired.org
...
https://github.com/kickingvegas/casual/blob/main/docs/ibuffer.org

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'ibuffer
    (my/bind-keys ibuffer-mode-map
...
https://github.com/kickingvegas/casual/blob/main/docs/info.org

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'info
    (my/bind-keys Info-mode-map
                  '(("<f10>" casual-info-tmenu))))
#+end_src

**** =casual-isearch=

https://github.com/kickingvegas/casual/blob/main/docs/isearch.org

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'isearch
    (my/bind-keys isearch-mode-map
                  '(("<f10>" casual-isearch-tmenu))))
#+end_src

**** =casual-make=

https://github.com/kickingvegas/casual/blob/main/docs/make-mode.org

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 'make-mode
    (my/bind-keys makefile-mode-map
                  '(("<f10>" casual-make-tmenu))))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load 're-builder
    (my/bind-keys reb-mode-map
                  '(("<f10>" casual-re-builder-tmenu)))
    (my/bind-keys reb-lisp-mode-map
                  '(("<f10>" casual-re-builder-tmenu))))
#+end_src
...
Des que vaig cambiar de =elpy= a =eglot= tinc la sensació que el
comportament de =company= ha canviat. Abans feia concordances per
prefix i ara sembla /fuzzy/.

En utilitzar el /transformer/ =company-sort-prefer-same-case-prefix=,
es col·loquen al principi de la lista els candidats que el nom comença
pel prefix. No m'acaba de convèncer, però és millor que el
comportament per defecte.

Imagino que aquest comportament afecta tots els modes on s'activa
=eglot=, no la resta. De moment ho configuro ací i, en funció del que
vegi, el mouré als /hooks/ dels modes afectats.
...
#+begin_src emacs-lisp :tangle yes
  (use-package company
    ...
    (setq company-tooltip-limit 20
          ...
          company-tooltip-annotation-padding 5
...
semblat una mica intrusiu. Per altra banda, =eglot= forma part d'Emacs
des de la versió 29. Començaré provant-lo i després decidiré.
...
*** =hydra=                                                      :deprecated:
:LOGBOOK:
- Note taken on [2025-08-20 dc. 08:37] \\
  =hydra= no sembla mantingut i mai l'he incorporat al meu workflow.
  Probablement es pugui aconseguir el mateix amb =transient=.

  Revisant la configuració he vist que defineixo algunes /hidres/, que
  mai he utilitzat. Es podrien esborrar o convertir a /transients/.
  També es parla de =hydra= en la configuració de =swiper=.

  Pel momento només el marco com /deprecated/, a l'espera de poder
  investigar una mica i veure que decideixo.
:END:
...
Mode per treballar amb arxius =justfile=.
...
Els arxius =.inc= que utilitzo pels include en ensamblador s'obren per
 defecte en =fundamental-mode=.
...
Tenim un problema semblant amb =:=, que sagna la línia si s'identifica
una etiqueta. La funció =asm-colon= original no identifica les
etiquetes locals i les sagna com instruccions.
  ...
  (defun asm-colon ()
    ...
    (let ((labelp nil)
      ...
      (save-excursion
        ...
        (setq is-local (skip-chars-backward "."))
        (skip-syntax-backward " ")
        (when (setq labelp (bolp))
          (delete-horizontal-space)
          (unless (= is-local 0)
            (insert (make-string my/asm-local-label-indent ? )))))
...
**** Referència detallada d'instruccions x86                     :desactivat:

#+begin_note
Aquesta referència de ensamblador x86 està extreta de la documentació
original de Intel, és molt detallada, pel meu cas d'us massa. La
mantinc per si en un futur em cal, però es desactiva.
#+end_note
...
**** Referència abreujada d'instruccions x86

Aquesta referència és menys detallada i m'agrada més:
...
Per defecte, quan =imenu= salta a una secció dins un buffer =org= es
mostren les capçaleres mínimes necessàries: la de destí i els pares,
no els germans. Em resulta confús i complica les coses si vull accedir
a seccions adjacents. Podria recordar que =C-u C-c C-r= les expandeix,
però prefereixo que ho recordi Emacs.

El codi següent instal·la la funció =my/org-fold-reveal-siblings= en
el hook =imenu-after-jump-hook=, localment en el buffer =org=, de
forma que no interfereix en altres modes.

#+begin_src emacs-lisp :tangle yes
  (defun my/org-fold-reveal-siblings ()
    (org-fold-show-set-visibility 'canonical)
    (org-fold-heading nil t))

  (with-eval-after-load 'org
    (add-hook 'org-mode-hook
              (lambda () (with-eval-after-load 'imenu
                      (add-hook 'imenu-after-jump-hook
                                #'my/org-fold-reveal-siblings
                                nil t)))))
#+end_src
...
Configuració de =eglot= per =python=.
...
**** =elpy=                                                      :desactivat:
:LOGBOOK:
- Note taken on [2025-08-20 dc. 08:58] \\
  Darrerament tinc la sensació de que =elpy= és molt intrussiu, que no
  evoluciona i hi ha altres alternatives que poden donar millor
  resultats.

  Per començar el desactivo, amb l'objectiu de veure que trobo a faltar.
:END:
...
Configura alguns aspectes del mode utilizat pel REPL de python.
...
#+begin_src emacs-lisp :tangle no
  ...
  (eval-after-load "python"
    '(progn
       (define-key python-ts-mode-map (kbd "C-c h") 'pylookup-lookup)))
...
#+begin_src emacs-lisp :tangle yes
  (use-package pytest
    ...
    (with-eval-after-load "python"
      (my/bind-keys python-ts-mode-map
                    '(("C-c m t a" pytest-all)
                      ("C-c m t t" pytest-run-tests-for-current-module))))
...
**** =python-ts-mode=                                               :builtin:

Remapeja =python-mode= per activar =python-ts-mode=.
...
Configura alguns aspectes de =python=.
...
**** =sphinx-doc=                                                :desactivat:
:LOGBOOK:
- Note taken on [2025-08-20 dc. 09:15] \\
  No tinc memòria d'haver-ho utilitzat mai. Probablement no sigui
  compatible amb els /type hints/. Ho desactivo.
:END:
...
Les comandes següents s'ha copiat de =elpy=. Es renombren afegint el
prefix =my/=:

#+begin_src emacs-lisp :tangle yes
  (defun my/elpy-nav-move-line-or-region-down (&optional beg end)
    ...
    (interactive
     (if (use-region-p)
       ...
       (list nil nil)))
    (if beg
        (my/elpy--nav-move-region-vertically beg end 1)
      (my/elpy--nav-move-line-vertically 1)))

  (defun my/elpy-nav-move-line-or-region-up (&optional beg end)
    "Move the current line or active region down."
    (interactive
     (if (use-region-p)
         (list (region-beginning) (region-end))
  ...
  (defun my/elpy--nav-move-line-vertically (dir)
  ...
  (defun my/elpy--nav-move-region-vertically (beg end dir)
...
Defineix un /transient/ per facilitar-me recordar les comandes de
navegació disponibles en python.

#+begin_src emacs-lisp :tangle yes
  (with-eval-after-load "python"
    (require 'transient)

    (transient-define-prefix my/python-nav-transient ()
      "Navegación estructural en Python."
      [["Defun"
        ("d" "Backward defun" python-nav-backward-defun :transient t)
        ("D" "Forward defun" python-nav-forward-defun :transient t)
        ("M-d" "Beginning of defun" treesit-beginning-of-defun)
        ("M-D" "End of defun" treesit-end-of-defun)]
       ["Block"
        ("b" "Backward block" python-nav-backward-block :transient t)
        ("B" "Forward block" python-nav-forward-block :transient t)
        ("M-b" "Beginning of block" python-nav-beginning-of-block)
        ("M-B" "End of block" python-nav-end-of-block)]
       ["Statement"
        ("s" "Backward statement" python-nav-backward-statement :transient t)
        ("S" "Forward statement" python-nav-forward-statement :transient t)
        ("M-s" "Beginning of statement" python-nav-beginning-of-statement)
        ("M-S" "End of statement" python-nav-end-of-statement)]
       ["Sexp"
        ("x" "Backward sexp safe" python-nav-backward-sexp-safe :transient t)
        ("X" "Forward sexp safe" python-nav-forward-sexp-safe :transient t)
        ("M-x" "Backward sexp" python-nav-backward-sexp :transient t)
        ("M-X" "Forward sexp" python-nav-forward-sexp :transient t)]
       ["List"
        ("l" "Backward up-list" python-nav-backward-up-list :transient t)
        ("L" "Up-list" python-nav-up-list :transient t)]]
      ))
#+end_src
  ...
  (with-eval-after-load "python"
    (my/bind-keys
     ...
     '(("C-c m m" pyx/make)
       ...
       ("M-S-<down>" my/elpy-nav-move-line-or-region-down)
       ("M-S-<up>" my/elpy-nav-move-line-or-region-up)
       ...
       ;; Redefineixo C-z (que normalment obre una shell) per obrir
       ;; l'interpret python.
       ("C-z" run-python)
       ("C-S-z" my/bash-terminal)
       ))

    (define-key python-ts-mode-map (kbd "C-<tab>")
    ...
    ;; Prefíx "C-c m n" per les comandes de navegació estructural
    (define-key python-ts-mode-map (kbd "C-c m n") 'my/python-nav-transient)
    )
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  (add-hook 'rst-mode-hook
            (lambda ()
              (auto-fill-mode 1)
              (flyspell-mode 0)
              ;; desactiva l'expansió de "con" -> ".. content::"
              (define-abbrev rst-mode-abbrev-table "con" "con" nil)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-to-list 'auto-mode-alist '("\\.vpy$" . python-ts-mode))
  (add-to-list 'auto-mode-alist '("\\.cpy$" . python-ts-mode))
...
*** Ressaltar parèntesis coincidents
...
*** Espais en blanc al final de línia
...
*** Títol de la finestra en mode gràfic
...
*** Comportament de majúcules/minúscules al ordenar
...
Configuració del /scroll/ vertical:

#+begin_src emacs-lisp :tangle yes
  (setq scroll-preserve-screen-position t)
  (customize-set-value 'scroll-step 1)
  (customize-set-value 'scroll-conservatively 10)
  (customize-set-value 'scroll-margin 0)
#+end_src

*** Per classificar

Opcions pendents de classificar en una secció:
...
Com =goto-line= però activa =display-line-numbers= per facilitar
identificar la línia.
...
#+begin_src emacs-lisp :tangle yes
  ...
  numbers."
    (interactive)
    (let ((saved-display-line-numbers-mode display-line-numbers-mode)
          (saved-display-line-numbers-type display-line-numbers-type))
      (unwind-protect
          (let ((display-line-numbers-type t))
            (display-line-numbers-mode 1)
            (call-interactively 'goto-line))
        (if saved-display-line-numbers-mode
            (progn
              (customize-set-value 'display-line-numbers-type saved-display-line-numbers-type)
...
*** =my/toggle-line-numbers-mode=

Activa/desactiva la numeració relativa de les línies.

#+begin_src emacs-lisp :tangle yes
  (defun my/toggle-line-numbers-mode ()
    "Activa la numeració de les línies."
    (interactive)
    (if (and (boundp 'display-line-numbers-mode)
             display-line-numbers-mode)
        (display-line-numbers-mode 0)
      (customize-set-value 'display-line-numbers-type 'relative)
      (display-line-numbers-mode 1)))
#+end_src
...
*** =my/simple-toggle-highlight-symbol-at-point=

https://www.jamescherti.com/emacs-symbol-highlighting-built-in-functions/

Ressalta el symbol sota punt. Habitualment utilitzo alguna variant de
cerca incremental per locaitzar els símbols però el ressaltat no és
permanent. Provaré aquesta solució a veure si em va bé i soc capaç
d'interioritzar-la.

#+begin_src emacs-lisp :tangle yes
  (require 'hi-lock)

  (defun my/simple-toggle-highlight-symbol-at-point ()
...
Ordena alfabéticament les paraules que apareixen dins la regió.

La idea de la comanda sorgeix de la necessitat d'ordenar
alfabèticament el nom dels arxius que apareixen en els prerequisits
d'un =Makefile= (en una variable utilitzada en la posició dels
prerequisits, per ser precís). En aquest context les paraules estan
separades per espais, però es pot especificar el separador en una
variable local (probablement depenent del mode).

#+begin_src emacs-lisp :tangle yes
  (defvar my/sw-separator " +"
    "Expressió regular utilitzada com separador de les paraules.")

  (defvar my/sw-joiner " "
    "Separador utilitzat al ajuntar les paraules una vegada ordenades.")

  (defun my/sort-words (start end)
    "Ordena les paraules de la regió. El comportament es pot
  configurar canviant el valor de les variables `my/sw-separator' i
  `my/sw-joiner'. El més habitual és configurar el seu valor
  depenet del mode."
    (interactive "r")
    (when (use-region-p)
      (let ((words (s-split my/sw-separator
                            (buffer-substring-no-properties start end))))
        (delete-region start end)
        (insert (s-join my/sw-joiner (sort words #'string<))))))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (global-set-key (kbd "C-o") #'arv/open-line-after-and-indent)
  (global-set-key (kbd "C-S-o") #'arv/open-line-before-and-indent)

  (global-set-key (kbd "C-w") #'my/kill-region)

  (global-set-key (kbd "M-D") #'my/dedent-to-point)

  (global-set-key (kbd "C-g") #'x-arv/keyboard-quit-dwim)
  ...
  ;; multi-line
  (global-set-key (kbd "C-c r m") (my/define-repeatable-command (kbd "m") 'multi-line))
  ...
  (global-set-key (kbd "C-c t h") #'my/simple-toggle-highlight-symbol-at-point)
  (global-set-key (kbd "C-c t l") #'my/toggle-line-numbers-mode)
...
** =x-arv/keyboard-quit-dwim=

https://emacsredux.com/blog/2025/06/01/let-s-make-keyboard-quit-smarter/

Aquesta és una versió millorada de =arv/abort-recursive-edit= que
adreça altres casos.

#+begin_src emacs-lisp :tangle yes
  (defun x-arv/keyboard-quit-dwim ()
