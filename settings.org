...
Els arxius de backup creats durant l'edició són més una molèstia que
altra cosa. Vaig a provar que tal em funciona guardar-los tots en un
lloc.
...
- en programes python funciona correctament però no en emacs-lisp
  (falla si el cursor està sobre un guió). Suposo que hauria
  d'investigar les /syntax table/.

- el separador està fixat al codi. Igual es podria configurar en cada
  mode.

Pel moment és suficientment bó. Si resulta útil acabaré de polir-la.
