...
una versió més recent. Aquesta secció defineix funcions que faciliten
mantindre la compatibilitat.

Emacs 24 no defineix =string-greaterp= (ni =string>=), curiosament sí
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; doom-one té alguns problemes de visualització en terminals
  ;; (svn-diff resulta il·legible)
...
La forma en que =doom-one= defineix les faces de =diff-hl= té
...
s'utilitza un color solid enlloc de ressaltar només la vora.
...
El color que he triat no em convenç però almenys facilita veure el
límit de les finestres. Una altra combinació utilitza el mateix groc
que utilitzo a i3 per ressaltar la finestra activa, actiu =#ffcc00= i
inactiu =#aa8900=.

Els colors per defecte de =term= no em convencen. Aquesta combinació
em convenç més:
...
Apanyo ràpid per facilitar compilar AGD.
...
Visita aquest arxiu de configuració:
