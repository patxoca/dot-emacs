...
Crec que la millor forma d'organitzar la configuració des les /faces/
...
En alguns temes (detectat en =doom-one=) la definició de les faces de
=diff-hl= té problemes quan el frame es parteix en dos verticalment,
en la finestra de la dreta no es visualitza correctament. Funciona
millor si s'utilitza un color solid enlloc de ressaltar només la vora.
...
El color que utilitza =transient= per ressaltar les opcions /actives/
no destaca gaire.
...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun arv/visit-emacs-config-directory ()
...
#+begin_src emacs-lisp :tangle yes
  ...
  (global-set-key (kbd "C-c j E") 'arv/visit-emacs-config-directory)
