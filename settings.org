...
Es defineix una comanda que mostra la referència. Es podria millorar
per buscar una instrucció al índex i mostrar una pàgina específica.

#+begin_src emacs-lisp :tangle yes
  (defun my/asm-x86-reference ()
    "Mostra la referencia de les instruccions del x86."
    (interactive)
    (eww "https://www.felixcloutier.com/x86/" t))
#+end_src
