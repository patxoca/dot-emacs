...
Darrerament acostumo a crar un arxiu =TODO.org= en l'arrel de cada
/projecte/. Aquesta comanda permer visitar-lo des de dins del
projecte.

En Hera utilitzo un arxiu TODO global que guardo dins un directori.
Això no encaixa amb aquesta comanda. Una possibilitat seria definir
una variable (entorn? dir-locals?) que especifica la ubicació. Queda
per definir l'ordre de prioritat si hi ha conflicte. Crec que el
millor es buscar i si no el troba consultar la variable. Un prefix pot
invertir l'ordre?
...
#+begin_src emacs-lisp :tangle yes
  (defun my/visit-current-project-TODO ()
    "Visita l'arxiu 'TODO.org' del projecte actual, per una
  definició relaxada de 'projecte."
    (interactive)
    (let ((directori (locate-dominating-file default-directory "TODO.org")))
      (when directori
        (find-file (f-join directori "TODO.org")))))
#+end_src
