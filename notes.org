...
Tinc que acabar d'aprendre com funciona, mentre tant ací hi ha un
resum de les comandes que em semblen més útils:

- =rst-adjust= (=C-c C-a C-a=): fa diferents ajustament sobre la
  decoració de les capçaleres:

  - afegeix la decoració si no en té, la tria de manera intel·ligent
    en funció del context.

  - ajusta longitud de la decoració si es curta

  - promou (degrada amb prefix) la capçalera en funció del valor de
    =rst-preferred-adornments=.

- =rst-adjust-region=: promou o degrada (amb prefix) les seccions dins
  la regió.

- =rst-display-hdr-hierarchy= (=C-c C-a C-d=): mostra la decoració
  utilitzad en cada nivell de capçaleres.

- =rst-straighten-sections= (=C-c C-a C-s=): actualitza (normalitza)
  la decoració de les capçaleres d'un document utilitzant els valors
  especificats en =rst-preferred-adornments=.

- =rst-toc= (=C-c C-t C-t=): crea un buffer que mostra la taula de
  continguts i permet navegar entre seccions, a la =imenu=.

- =rst-insert-list=: insereix una llista nova o un element dins la
  llista. Pregunta el tipus. Inserir un nou element sembla que només
  funciona si estem en la primera línia.

- =rst-forward-section= (=C-M-e=): personalitzat com =M-<down>=

- =rst-backward-section= (=C-M-a=): personalitzat com =M-<up>=. La
  combinació de tecles predefinida no funciona en el portàtil. No tinc
  clar qui la intercepta, imagino que i3 però no aconsegueixo entendre
  que fa ni on es defineix.

- =rst-enumerate-region= (=C-c C-l C-e=),
  =rst-bullet-list-region= (=C-c C-l C-b=): converteixen els paràgrafs
  de la regió en element d'una llista numerada i normal
  respectivament. Amb prefix converteixen totes les línies.

- =rst-convert-bullets-to-enumeration= (=C-c C-l C-c=): converteix els
  elements dins la regió d'una llista normal en enumerats.
...
** PAUS =imenu= en =settings.org=                                 :annoyance:
:PROPERTIES:
:ID:       1119f95b-e995-4328-837b-b6b2929c97a4
:END:
:LOGBOOK:
- State "PAUS"       from "TODO"       [2025-08-23 ds. 21:52] \\
  Des d'un temps cap ací el comportament a revertit màgicament al que
  vull. He estat tocant la configuració de =org=, però no soc conscient
  d'haver tocat res que pugui influir en aquest canvi.

  La única cosa és que vaig treure les variables /buffer local/ de
  =settings.org=, i una estava relacionada amb la profonditat que visita
  =imenu=.

  Pel moment ho deixo pausat a l'espera de confirmar que és estable.
:END:

He començat (agost 2025) a utilitzar =imenu= (=C-c j s=) per saltar
directament a seccions de documents org. Darrerament estic revisant la
configuracó d'Emacs i em cal accedir sovint a la secció /Configuració/
> /Modes/. He detectat que =imenu= no mostra aquesta secció. Alguna
vegada, seleccionant =*Rescan*=, crec que ha aparegut, potser he
accedit manualment a la secció abans del =*Rescan*=. No és un tema de
límit de profunditat, /Tema/ i /Paquets/ son dos seccions al mateix
nivell (2) que /Modes/, i apareixen.

Una altra cosa estranya és que, de vegades, el comportament de =imenu=
és motrar les capçaleres de primer nivell, en seleccionar-ne una
mostra les subcapçaleres etc., mentre que altres vegades (la majoria)
mostra una llista plana amb totes les capçaleres. El primer és el
comportament que voldria sempre, en el segon (l'habitual) és on es
manifesta l'error.

El cas és que resulta molest. Com que he començat a utilitzar-ho fa
poc no sé si és un error /normal/ o està relacionat amb els últims
canvis que he fet a la configuració.

** PAUS cerca =fuzzy= en =company=                                :annoyance:
:LOGBOOK:
- State "PAUS"       from "TODO"       [2025-08-23 ds. 21:17] \\
  La meva comprensió del funcionament de =company= és limitada. Dit açò,
  aparentment el problema està causat pel /backend/ =company-capf=. No
  he trobat cap manera de configurar aquest /backend/ per influir en la
  seva sortida. La solució que he trobat és aplicar el /transformer/
  =company-sort-prefer-same-case-prefix=. Aquest /transformer/ mou al
  principi de la llista els candidats que el nom comença pel prefix i
  deixa al final els que encaixen de manera /fuzzy/.

  No és perfecta, té comportament estranys, però és molt millor que el
  comportament original.

  Pel moment deixo el TODO pausat, a l'espera de veure si la solució és
  suficientment satisfactòria o si trobo algo millor.
:END:

Al completar en python (no provat en altres), =company= utiliza /fuzzy
matching/. Està bé quan no sé el que busco, però la gran majoria de
vegades el que estic escribint és el prefix. No seria un gran problema
si no fos perqué al premer =TAB= per completar el que fa es ciclar
entres les opciones, no completar/filtrar pel prefix comú.

Crec que la solució ideal seria utilizar /matching/ per préfix per
defecte i definir una combinació de tecles per activar el mode
/fuzzy/. Una altra possibilitat seria que =company= mostrés les
coincidències per prefix al principi i les /fuzzy/ al final.

No ho havia detectat fins ara (agost 2025). Abans, potser =elpy=
configurava alguna cosa de =company= i al canviar a =eglot= s'utilitza
el comportament per defecte, o simplement és cosa de com el /backend/
(=jedi=, =python-lsp-server=) fa la cerca. El segon sembla que té més
sentit.

** TODO Actualitzar =pyx.el=

Definir =pyx/just=, semblant a =pyx/make=, però per =just=.

Crec recordar qué, algunes implementacions de funcionalitats
relacionades amb descobrir l'estructura del programa eren fràgils.
Mirar si es poden millorar utilitzant funcionalitats de =tree-sitter=.

** TODO =rg= i buffer =org-mode= col·lapsat                       :annoyance:

Quan es visita una ocurrències del resultat d'una cerca =rg= i el
destí es troba en una secció col·lapsada d'un buffer =org=, el cursor
es queda a la capçalera de la secció, no l'expandeix de la mateixa
forma que fa =occur=.

La solució proposada per =occur= (pensava que passava el mateix i li
he preguntat) per ChatGPT és:

#+begin_src emacs-lisp
  (defun my-occur-reveal-in-org ()
    "Tras saltar a una ocurrencia en org, revelar la entrada."
  ...
  (add-hook 'occur-hook
            (lambda ()
              (add-hook 'occur-mode-find-occurrence-hook
                        #'my-occur-reveal-in-org)))
#+end_src

Investigant el /hook/ =occur-mode-find-occurrence-hook=, veig que
apareix la funció =org-fold-reveal=.

Mirant el codi de =rg.el=, sembla que utilitza un buffer de compilació
normal i corrent, no defineix cap /hook/ que s'activa al saltar a un
resultat de la cerca. S'utilitzen les comandes
=compilation-next-error= i =compilation-previous-error= per visitar
les coincidències.

No veig una forma neta de resoldre-ho. El que se m'acut es afegir
advice a les comandes, que s'executi al final (quan el cursor està en
la posició de destí), comprovi si és un buffer =org= i, en cas
afirmatiu, executi =org-fold-reveal= (=org-reveal= és un àlias).

** TODO Revisar /keybindings/ de =paredit=                        :annoyance:

Fa temps que no programa en elisp, i potser ho recordo mal, però diria
que =C-<left>= i =C-<righ>= eren comandes de navegació. Ara son
comandes d'edició estructural.

Una altra molèstia és que m'estic acostumant a utilitzar el /keymap/
=M-s= (=occur= i =highlight=), i =paredit= el segresta.
...
- [[https://github.com/p3r7/awesome-elisp][Awesome Elisp]]: a list of resources linked to Emacs Lisp (Elisp)
  development.

- [[https://lmno.lol/alvaro/modern-elisp-libraries][Modern Emacs lisp libraries]]: enumera algunes llibreries.

- [[https://www.jamescherti.com/essential-emacs-packages/][Essential Emacs Packages for Efficient Software Development and Text
  Editing]]: post que llista una serie de paquets.
...
- [[https://github.com/wolray/symbol-overlay][symbol overlay]]:Highlight symbols with overlays while providing a
  keymap for various operations about highlighted symbols.

  Per la descripció és semblant a =highlight-symbol-at-point=, que no
  sap de l'estructura del codi. El que busco és una comanda que mostri
  els simbols locals al context actual.

- [[https://github.com/bling/fzf.el][fzf]]: fins ara no m'havia plantejat algo tant bàsic com poder buscar
  un arxiu des de dins d'Emacs. L'opció =find-name-dired= funciona,
  però és molt cutre i no em deixa obrir l'arxiu, que seria l'objectiu
  principal.

  No és exactament el que busco, però =fzf= sembla interessant.

- [[https://github.com/konrad1977/flyover][flyover]]: A modern, aesthetic overlay display for Flycheck and
  Flymake in Emacs.

  Interpreto que mostra informació d'errors de flycheck/flymake en
  línea enlloc de en un buffer separat o el minibuffer.

- [[https://github.com/karthink/gptel][gptel]]: gptel is a simple Large Language Model chat client for Emacs,
  ...
  Em permetria, per exemple, crear una comanda per automatizar la
  revisió dels missatges de commit.
...
- [[https://emacs.dyerdwelling.family/emacs/20250719094720-emacs--building-an-emacs-cursor-heatmap-pt1/][Building a New Package From Scratch]]: serie de posts on explica com
  crear un paquet elisp.
...
Açò és una nota que tenia en google keep i que vull arxivar, però
potser hi ha algo interessant, que probablemente mai miraré 😺.

- https://www.youtube.com/watch?v=Oqn56p2cGQ4
...
- systmem crafters:
  ...
  - https://discord.com/invite/yVD8Gx6

- auto-package-update-before-hook: sembla que s''executa abans
  d''actualitzar els paquets. Es podria utilitzar per fer un backup.
  És un paquet extern.

- https://karthinks.com/software/bridging-islands-in-emacs-1/ integra
  re-builder dins *-replace-regex. No especialment útil per si mateix
  però interessant com integra funcionalitats existents.
...
** DONE Comportament de =:= en les etiquetes locals             :bug:asmmode:

- State "DONE"       from "TODO"       [2025-02-07 dv. 10:44]

La funció [[file:/opt/emacs/29.4.1/share/emacs/29.4/lisp/progmodes/asm-mode.el.gz::defun asm-colon (][asm-colon]] assumeix que una etiqueta està formada per
lletres, dígits i subratllat (sintaxi =w=) i no gestiona correctament
les etiquetes locals =NASM= (comencen amb un punt).

He tingut un problema semblat al sagnar les etiquetes i l'he resolt
afegint una comprovació per identificar les etiquetes locals. Es
podria aplicar una solució semblant, però estic pensant que també es
podria ampliar la taula de sintaxi de =w= afegint el punt. Açò podria
fer innecessari el canvi per sangar.

Les etiquetes locals es sagnen a nivell 2, no 0, per mostrar el
nidament. Caldria valorar si ho vull mantindre.
  ...
  (defun asm-colon ()
    ...
    (let ((labelp nil))
      ...
      (when labelp
        ...
        (tab-to-tab-stop))))
#+end_src
** [7/7] Varis noves funcionalitats en =python-ts-mode= + =eglot=
*** DONE [#A] Activació del REPL python
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-22 dv. 19:26] \\
  Es canvia. S'aplica el mateix canvi a =lisp-mode= per obrir =ielm=.
:END:

Crec que mai seré capaç d'interioritzar la combinació =C-c C-p= per
obrir l'interpret de python. Trobo més natural utilizar =C-z= (que per
defecte obre una terminal) i, potser, relegar la terminal a =C-Z= en
els modes que obren un REPL.

*** DONE [#C] Comportament estrany de =imenu=
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-22 dv. 20:43] \\
  No el faig servir tant com per a que sigui una molèstia, i de fet, no
  sé si és una molèstia. Si l'afegeixo al workflow i no m'agrada, ja em
  barallaré.
:END:

Abans, demanava triar un objecte de nivell superior i permetia saltar
a l'objecte o seleccionar un subobjecte. Ara, mostra algun(s)
objecte(s) de nivel superior (no tinc clar si l'actual o altres
també), i categories d'objectes (classe, métode, funció ...).

Crec que el comportament original s'adapta millor a la meva forma de
pensar.

Veure [[id:1119f95b-e995-4328-837b-b6b2929c97a4][=imenu= en =settings.org=]].

*** DONE [#B] Gestió d'imports
:LOGBOOK:
- State "DONE"       from "PAUS"       [2025-08-22 dv. 22:58] \\
  Hauré d'investigar com afegir =import= automàticament al codi, a una
  mala puc utilizar la meva comanda =pyimport=.

  Respecte a que els imports no s'ordenin al guardar l'arxiu, el
  problema és de la pròpia funció =py-isort-before-save=. Com que
  s'afegeix a un /hook/ global, abans de fer res comprova que el mode
  del buffer sigui =python-mode=. Com que ara utilitzo =python-ts-mode=,
  ha deixat de funcionar.

  Tinc =py-isort.el= en local, així que l'he modificat.

  Hi ha un problema semblant en =pylint.el=, no es té en compte
  =python-ts-mode=. Crec que no utilitzo les funcions afectades, però ja
  que hi soc, les corregeixo.
- State "PAUS"       from "TODO"       [2025-08-22 dv. 21:21] \\
  La funcionalitat dels import oferida per =python-mode= es molt bàsica,
  no és el que busco.

  Respecte a que =isort= no ordena els imports al guardar, el problema
  és que =eglot= fa que =before-save-hook= sigui buffer local,
  instal·lant la seva funció, amb el que les funcions instal·lades
  globalment no s'executen, =py-isort-before-save= entre elles.

  He d'acabar de confirma que el culpable és =eglot=. Desactivant el
  bloc de codi que l'activa en la configuració de python, el problema
  desapareix. Si l'activo manualment, el problema apareix. El que em
  despista és que al codi de =eglot= només hi ha dos referències a
  =before-save-hook=, en cap es fa local.
:END:

Alguna de la funcionalitat seguent de python-ts-mode no funciona.
...
Al guardar l'arxiu no es reordenen el emports amb =isort=.

*** DONE [#B] Estudiar funcionalitat per navegar pel codi
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-22 dv. 18:54] \\
  Es defineix un transient. Açò facilita recordar les comandes i permet
  repetir les comandes prement una tecla.
:END:
...
Les comandes següents no estan assignades a cap tecla, no crec que
siguin d'us freqüent, per tant no cal assignar-les a combinacions
curtes. Valorar si afegir-les al mapa de salt (=C-m j=):
...
*** DONE [#A] =word-mode= i =python-ts-mode=
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-22 dv. 20:21] \\
  L'activació de =superword-mode= (i altres coses) s'afegia al hook
  =python-mode-hook=, amb el que no s'executava. S'afegeix al hook
  =python-ts-mode-hook=.
:END:
*** DONE [#A] =eglot= no s'ha activar al carregar =ecs.py=
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-22 dv. 20:22] \\
  El problema ací era que l'activació d'eglot (=eglot-ensure=) estava
  protegida per un =with-eval-aftere-load 'eglot=, amb el que mai
  s'executava (fins que no es carrega =eglot= no s'activa, pero si no
  s'activa no es carrega).
:END:

Al obrir =ecs.py= no s'ha activat =eglot= automàticament.

*** DONE [#C] Desplaçar codi
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-22 dv. 23:43] \\
  Les dos comandes únicament utilitzen dos funcions auxiliars.
  S'incorporen les 4 a la configuració.
:END:

Ocasionalment utilitzava =elpy-nav-move-line-or-region-down= i
=elpy-nav-move-line-or-region-up= per moure codi amunt i avall.

Mirar si és complicat de portar a la nova configuració. Açò es pot
considerar /edició estructurada/, potser =tree-sitter= ofereix
funcionalitats.
...
#+end_src
** DONE Warnings en =pyx.el=
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-08-23 ds. 01:06] \\
  La funció =subseq= s'ha renombrat com =cl-subseq=, es corregeix. La
  resta de warnings estan causats perqué no requereixo ='python=.
:END:

Revisar aquests warnings:

#+begin_src text
  ⛔ Warning (native-compiler): ~/.emacs.d/elpa/arv-py-20220206.1743/pyx.el:514:26: Warning: the function ‘subseq’ is not known to be defined.
  ⛔ Warning (native-compiler): ~/.emacs.d/elpa/arv-py-20220206.1743/pyx.el:220:16: Warning: the function ‘python-syntax-comment-or-string-p’ is not known to be defined.
  ⛔ Warning (native-compiler): ~/.emacs.d/elpa/arv-py-20220206.1743/pyx.el:193:4: Warning: the function ‘python-nav-up-list’ is not known to be defined.
  ⛔ Warning (native-compiler): ~/.emacs.d/elpa/arv-py-20220206.1743/pyx.el:178:6: Warning: the function ‘python-nav-beginning-of-defun’ is not known to be defined.
  ⛔ Warning (native-compiler): ~/.emacs.d/elpa/arv-py-20220206.1743/pyx.el:95:5: Warning: the function ‘python-info-looking-at-beginning-of-defun’ is not known to be defined.
#+end_src
