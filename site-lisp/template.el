...
(defvar template-default-expansion-alist
  '(("POINT" (setq template-point (point-marker))) ; point
    ...
    ("FILE_UPCASE" (insert (upcase (nth 2 template-file))
			   (nth 3 template-file)))
    ("FILE_DOWNCASE" (insert (downcase (nth 2 template-file))
			   (nth 3 template-file)))
    ("FILE_CAPITALIZE" (insert (capitalize (nth 2 template-file))
			   (nth 3 template-file)))
...
(defun template-derivation-original (full arg &optional no-default)
...
(defun arv-filter (predicat llista)
  (let ((res '())
        memb)
    (while (setq memb (pop llista))
      (if (funcall predicat memb)
            (setq res (append res (list memb)))))
    res))

(defun arv-filtra-plantilles-directori (directori extensio)
  "carrega llista arxius de DIRECTORI que acaben anb
.EXTENSIO.tpl. Retorna una alist que mapeja el nom de l' arxiu (sense
.tpl) amb la seva ruta. Si no hi ha cap arxiu retorna la llista
buida."
  ;; faltaria filtrar els arxius per al que l'usuari no te permis de
  ;; lectura
  (and (file-accessible-directory-p directori)
       (mapcar (lambda (x) (cons (file-name-sans-extension (file-name-nondirectory x))
                                 x))
               (arv-filter 'file-readable-p
                           (directory-files directori t (concat "." extensio ".tpl$"))))))

(defun arv-template-derivation (full arg &optional no-default)
  "Busca l'arxiu en els directoris Template que troba mentres puja
fins l'arrel.
"
  (let ((extensio   (or (file-name-extension full) ""))
        (directori  (directory-file-name (file-name-directory full)))
        (plantilles ())
        (resultat   ()))
    (while (not (string-equal directori "/")) ; compte! ignora /Templates (a l'arrel)
      (let ((temp (arv-filtra-plantilles-directori (concat directori "/Templates")
                                                   extensio)))
        (while (not (null temp))
          ;;@(message " temp = %s\n plantilles = %s" temp plantilles)
          (if (not (assoc (caar temp) plantilles))
              (setq plantilles (append plantilles (cons (car temp) '()))))
          (setq temp (cdr temp)))
        (setq directori (directory-file-name (file-name-directory directori)))))
    ;; faltaria eliminar els duplicats

    ;; no m'acaba d'agradar que al editar un template
    ;; (~/Templates/foo.py.tpl p.e.) tambe proposa utilitzar un
    ;; template

    ;; inicialitza variable utilitzada pel mecanisme d'expanssio de
    ;; "variables"
    (setq template-file (template-split-filename full))

    (setq resultat (completing-read "Select a template: " plantilles))
    (if (null resultat) ; s'ha cancelat completing-read
        (cons full nil)
      (cons full (cdr (assoc resultat plantilles))))))



(defun template-derivation (full arg &optional no-default)
  ;;(message ">>> template-derivation")
  ;;(message "full = %s, arg = %s, no-default = %s" full arg no-default)
  ;;(let ((res (template-derivation-original full arg no-default)))
  (let ((res (arv-template-derivation full arg no-default)))
    ;;(message "<<< template-derivation")
    ;;(message "<<< refined = %s, template = %s" (car res) (cdr res))
    res))
