;;; Aquest arxiu és una copia del subministrar per elpy.
;;;
;;; Per evitar conflictes s'han renombrat les variables i funcions.

(defvar my/elpy-python-6 nil
  "Si t es genera codi que funciona amb python 2 i 3, si nil la
sintaxi depen de la versió de l'interpret que s'està
utiltizant.")
(put 'my/elpy-python-6 'safe-local-variable #'booleanp)

(defvar my/elpy-snippet-split-arg-arg-regex
...
(defvar my/elpy-snippet-split-arg-separator
"[[:blank:]]*,[[:blank:]]*"
"Regular expression matching the separator in a list of argument.")

(defun my/elpy-snippet-split-args (arg-string)
  "Split the python argument string ARG-STRING into a tuple of argument names."
  (mapcar (lambda (x)
            (when (string-match my/elpy-snippet-split-arg-arg-regex x)
              (match-string-no-properties 1 x)))
          (split-string arg-string my/elpy-snippet-split-arg-separator t)))

(defun my/elpy-snippet-current-method-and-args ()
...
(defun my/elpy-snippet-init-assignments (arg-string)
  ...
  (let ((indentation (make-string (save-excursion
    ...
    (mapconcat (lambda (arg)
               ...
               (my/elpy-snippet-split-args arg-string)
               "")))

(defun my/elpy-snippet-super-form ()
  "Return (Class, first-arg).method if Py2.
Else return ().method for Py3."
  (let* ((defun-info (my/elpy-snippet-current-method-and-args))
    ...
    (if (or my/elpy-python-6 (string-match py-version-num "2"))
        (format "(%s, %s).%s" class first-arg method)
      (format "().%s" method))))

(defun my/elpy-snippet-super-arguments ()
  "Return the argument list for the current method."
  (mapconcat (lambda (x) x)
             (cdr (nth 2 (my/elpy-snippet-current-method-and-args)))
             ", "))
