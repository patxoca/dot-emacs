...
def filtre_arxiu(dir, arxiu) :
    for ext in ('el', 'py', 'txt') :
        if arxiu.endswith('.' + ext) :
            return True
    return False

def filtre_dir(dir1, dir2) :
    return dir2 == 'kk'

init()

print file_walker('.', filtre_arxiu, filtre_dir)
...
oldbuff = lisp.current_buffer()
buffer = lisp.generate_new_buffer("*todo*")
lisp.set_buffer(buf)

# fes coses

lisp.set_buffer(oldbuff)
